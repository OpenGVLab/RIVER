...
def longshort_memory_streaming_breakpoint_infer(qa_mode = "breakpoint"):
    ...
    save_path = f"./output/offline/"+ sys.argv[1] +"/"  
    save_path = f"./output/offline_asr/"+ sys.argv[1] +"/"
    ...
    op_system="Carefully watch the video and pay attention to the cause and sequence of events, the detail and movement of objects, and the action and pose of persons. Based on your observations, give the answer that accurately addresses the question.\n"
    op_question_prompt=None
    op_answer_prompt="Answer:"
    ...
    with open(save_json,"a") as f:
        for idx in tqdm(range(len(json_file))[ext_len:]):
            ...
            if data_anno['q_num'] % num_tasks == int(gpu_id):
                ...
                num_segments = 16
                if num_segments != 0:
                    frame_indices = get_middle_index(q_time, num_segments)

                chat.messages.append([chat.roles[0], f"<Video><VideoHere></Video>\n"])
                op_chat.messages.append([op_chat.roles[0], f"<Video><VideoHere></Video>\n"])
                # op_chat.messages.append([op_chat.roles[0], short_time_prompt.format(current_time, round(q_time/fps, 2))+f"<Video><VideoHere></Video>\n"])  # ugly TODO
                ...
                video = transform(frames).to("cuda:0") # [586, 3, 224, 448]
                print(type(video), video.shape, video.dtype)
                T_, C, H, W = video.shape

                with torch.no_grad():
                    # video_emb, _, _ = model.encode_img(batch_video, [system], only_global=True)#!
                    video_emb, _, _ = model.encode_img([video], [system]) #len=1
                    print("videoembedd",video_emb[0].shape)
                short_video_emb = video_emb[0] # [1, 96, 4096]
                video_list = [short_video_emb]
                ...
                op_question = f"Question: {anno_question}\n"
                op_question = op_question.rstrip()
                # op_prompt = op_system + op_question + op_question_prompt
                op_prompt = op_system + op_question
                ask(op_prompt, op_chat)
                open_llm_message = answer(
                    conv=op_chat, model=model, do_sample=False, 
                    img_list=video_list, max_new_tokens=100, 
                    answer_prompt=op_answer_prompt, print_res=True,
                )[0]
                ...
                # new_item={"q_num":data_anno["q_num"],"gt_answer_id":gt_id,"pred_id":llm_message,"gt_answer":gt,"question":anno_question}
