    ...
    def encode_img(self, image, instruction):
        ...
        with self.maybe_autocast():
            ...
            for idx, img in enumerate(image):
                ...
                ).permute(0, 3, 5, 1, 2, 4, 6).reshape(-1, T, 3, self.local_size, self.local_size).contiguous() #(num_clips, T, 3, L, L)
            ...
            input_imgs = torch.cat(input_imgs, dim=0) #[2, 16, 3, 224, 224]+[1, 16, 3, 224, 224]
            ...
            image_embeds = self.vision_encoder(input_imgs, use_image) #encode([3, 3, 16, 224, 224]) -> [3, 16, 196, 1024]
            ...
            query_tokens = query_tokens.expand(image_embeds.shape[0], -1, -1) #这个根据hd模式产生多少个patch，复制几份query，每组得到96*4096
            ...
            # print("qformer_features.shape",qformer_features.shape)
        ...
        # print("output_imgs[0].shape",output_imgs[0].shape)
