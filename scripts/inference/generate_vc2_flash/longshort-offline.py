...
pretrained = "OpenGVLab/VideoChat-Flash-Qwen2-7B_res224"
model_name = get_model_name_from_path(pretrained)
device = "cuda"
device_map = "auto"
tokenizer, model, image_processor, max_length = load_pretrained_model(pretrained, None, model_name, torch_dtype="bfloat16", multimodal=True, attn_implementation="flash_attention_2")
model.eval()
cfg_pretrained = AutoConfig.from_pretrained(pretrained, trust_remote_code=True)

frame_aspect_ratio = model.config.frame_aspect_ratio if hasattr(model.config, "frame_aspect_ratio") else ""
...
save_path = f"./output/longshort/0922-{window_size}/"+ sys.argv[1] +"/"
save_path = f"./output/longshort/0925-off-{window_size}/"+ sys.argv[1] +"/"
save_path = f"./output/longshort/1124-offasr-{window_size}/"+ sys.argv[1] +"/"
...
# 新增或修改的参数设置
fixed_num_frames = window_size
window_size=fixed_num_frames
# 在主循环中进行修改
with open(save_json,"a") as f:
    for idx in tqdm(range(len(json_file))[ext_len:]):
        ...
        if data_anno['q_num'] % num_tasks == int(gpu_id):
            ...
            # 确保不会超出视频长度
            # frame_indices = np.linspace(max(q_time-fixed_num_frames+1, 0), q_time, fixed_num_frames, dtype=int)
            # frame_indices = np.linspace(0, real_num_frames-1, fixed_num_frames, dtype=int)
            # frame_indices=get_middle_index(real_num_frames,fixed_num_frames)
            frame_indices=get_middle_index(q_time,fixed_num_frames)
            ...
            if "anyres" in frame_aspect_ratio:
                print("use anyres_nopad")
                video = process_anyres_video_nopad(frames, image_processor, model.config.frame_grid_pinpoints, max_resolutions=model.config.max_num_pixels // len(frames)).half().cuda()
            else:
                video = image_processor.preprocess(frames, return_tensors="pt")["pixel_values"].cuda().half()
            ...
            # prompt = system + get_asr(video_path.split('/')[-1].split('.')[0],data_anno["question_time"])+question + question_prompt + answer_prompt
            ...
            open_conv.append_message(open_conv.roles[0],  DEFAULT_IMAGE_TOKEN + "\n")
            ...
            open_prompt = system + open_question + open_answer_prompt
