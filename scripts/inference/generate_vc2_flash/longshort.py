...
with open(save_json,"a") as f:
    for idx in tqdm(range(len(json_file))[ext_len:]):
        ...
        if data_anno['q_num'] % num_tasks == int(gpu_id):
            ...
            # breakpoint()
            frames = frames.asnumpy()                       #(586, 720, 1280, 3) #(327, 360, 640, 3)
            ...
            video_features = long_video_emb_list + short_video_emb_list #torch.Size([1, 4096, 3584]) torch.Size([1, 256, 3584])
