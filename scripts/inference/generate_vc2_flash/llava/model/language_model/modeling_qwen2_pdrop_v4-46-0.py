...
from transformers.models.configuration_qwen2 import Qwen2Config
...
class Qwen2Model_Pdrop(Qwen2PreTrainedModel):
    """
    ...
    def pdrop_rank_drop(
        ...
        position_ids, attention_mask, position_embeddings, cache_position, labels
    ):
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            if 'attention' in llm_compress_type:
                ...
                # attention_mask  
                eager_attention_mask = self._prepare_4d_causal_attention_mask_with_cache_position(
                    attention_mask,
            ...
            for i in range(batch_size):
                ...
                if 'attention' in llm_compress_type:
                    ...
                    if llm_compress_type == 'segment8_attention':
                        # 先分段然后再选
                        segment_len = 8 * 169
                        num_segments = len(attention_avg_text) // segment_len
                        start_index = torch.arange(0, len(attention_avg_text), segment_len).reshape(-1, 1).to(attention_avg_text.device)
                        top_rank_index = (attention_avg_text.reshape(num_segments, segment_len).topk(keep_length[i] // num_segments).indices + start_index).flatten(0, 1)
                    elif llm_compress_type == 'segment16_attention':
                        # 先分段然后再选
                        segment_len = 16 * 169
                        num_segments = len(attention_avg_text) // segment_len
                        start_index = torch.arange(0, len(attention_avg_text), segment_len).reshape(-1, 1).to(attention_avg_text.device)
                        top_rank_index = (attention_avg_text.reshape(num_segments, segment_len).topk(keep_length[i] // num_segments).indices + start_index).flatten(0, 1)
                    elif llm_compress_type == 'segment32_attention':
                        # 先分段然后再选
                        segment_len = 32 * 169
                        num_segments = len(attention_avg_text) // segment_len
                        start_index = torch.arange(0, len(attention_avg_text), segment_len).reshape(-1, 1).to(attention_avg_text.device)
                        top_rank_index = (attention_avg_text.reshape(num_segments, segment_len).topk(keep_length[i] // num_segments).indices + start_index).flatten(0, 1)
                    elif llm_compress_type == 'attention_segment8':
                        # rank and drop by attention score
                        segment_len = 8 * 169
                        num_segments = len(attention_avg_text) // segment_len

                        segment_top_rank_index = attention_avg_text.reshape(num_segments, segment_len).float().mean(dim=1).topk(keep_length[i] // segment_len).indices

                        top_rank_index = torch.tensor([[j*segment_len +i for i in range(segment_len) for j in segment_top_rank_index]]).to(attention_avg_text.device)

                    elif llm_compress_type == 'attention_segment16':
                        # rank and drop by attention score
                        segment_len = 16 * 169
                        num_segments = len(attention_avg_text) // segment_len
                        ...
                        top_rank_index = torch.tensor([j*segment_len +i for i in range(segment_len) for j in segment_top_rank_index])

                    elif llm_compress_type == 'attention':
            ...
            position_embeddings = self.rotary_emb(new_input_embeds, position_ids) # NOTE 应该是需要更新的


            cache_position = torch.arange(
                            0, new_input_embeds.shape[1], device=new_input_embeds.device
                        )


            return position_ids, new_attention_mask, new_input_embeds, position_embeddings, cache_position, new_labels
...
class Qwen2ForCausalLM_Pdrop(Qwen2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2Model_Pdrop(config)
        self.vocab_size = config.vocab_size
