...
def select_best_resolution(original_size, possible_resolutions, max_resolutions, patch_size=224):
    """
...
def process_anyres_image_nopad(image, processor, grid_pinpoints):
    ...
    patches = divide_to_patches(image.resize(best_resolution), processor.crop_size["height"])
...
def process_anyres_video_nopad_old(video, processor, grid_pinpoints):
    """可能太慢了这个实现
    Process an image with variable resolutions.
    ...
    for i in range(len(video)):
        image = Image.fromarray(video[i], mode='RGB')
        if best_resolution is None:
            best_resolution = select_best_resolution(image.size, possible_resolutions) # 所有视频帧size都是一样的
        # image_padded = resize_and_pad_image(image, best_resolution)
        ...
        # print("image.size", image.size, "len(image_patches):",  len(image_patches), "patch_size:", image_patches[0].shape)
        frame = torch.stack(image_patches, dim=0)
        frames.append(frame)
    
    frames = torch.stack(frames, dim=0).view(num_frames, len(image_patches), 3, patch_size, patch_size).permute(1, 0, 2, 3, 4).flatten(0, 1)

    # print(f"anyres num_frames: {num_frames}")
    # print(f"anyres frames.shape: {frames.shape}")
    return frames
...
def process_anyres_video_nopad(video, processor, grid_pinpoints, max_resolutions):
    ...
    best_resolution = select_best_resolution(video[0].shape[0:2], possible_resolutions, max_resolutions=max_resolutions, patch_size=patch_size) # 所有视频帧size都是一样
    # print("data: old_video.shape:", video.shape)
    # max_num_patches = 
    # max_num_pixels = 512*224*224
    # max_spatial_pixels = max_num_pixels // len(video)
    # 
